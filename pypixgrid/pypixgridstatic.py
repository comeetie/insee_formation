...
class MbTileWriter:
	...
	fields = dict(sum([map(lambda d:(d["name"],"Number"),options["data_format"]["context_variables"]),[("id","String"),("geosjon","String"),("area","Number")]],[]))	
	...
	self.cursor.execute("INSERT INTO metadata VALUES ('{vname} variables ranges zoom {z}', {v})".format(v="'"+json.dumps(map(lambda r:r["value"],rangeo))+"'",vname=vname,z=sc))
	...
	if(self.options["format"]=='pbf'):
		yfliped = 2**z-1-y 
		self.cursor.execute(sql,[z,x,yfliped,sqlite3.Binary(deflate(str(tile["tile"])))])
    def commit(self):
	self.conn.commit()
